...
///
/// The signing is deterministic (all-zero BIP340/aux randomness). The same
/// private key and message always produce the same signature.
///
/// Throws [StateError] in the astronomically unlikely event that the derived
/// nonce is zero (this cannot happen in practice with a valid private key).
Uint8List kaspaSchnorrSign(Uint8List privKey32, Uint8List msg32) {
  ...
  // Deterministic nonce — all-zero aux randomness (safe for signing).
...
///
/// Parameters:
///   - [txVersion]: transaction version (typically 0)
///   - [txIds]: hex transaction ID for each input outpoint
///   - [indices]: output index for each input outpoint
///   - [utxos]: UTXO data for each input (amount, script version, script)
///   - [outputs]: output data (value, script version, script)
///   - [inputIndex]: which input to compute the sighash for
///
/// Returns a 32-byte sighash.
...
/// Builds the P2PK signatureScript:
/// `OP_DATA_65 (0x41)` + 64-byte Schnorr signature + `SIG_HASH_ALL (0x01)`.
///
/// The returned bytes are 66 bytes long.
...
///
/// Returns `(version: 0, script: empty)` for strings shorter than 4 hex chars.
({int version, Uint8List script}) parseCompactSpk(String compactHex) {
  final bytes = Uint8List.fromList(HEX.decode(compactHex));
